<?php
    ...
    {
        const EXT = '.php';
        const HAX = '.hax'.self::EXT;
        protected $template_dir = './resources/views/';
        protected $template_cache_dir = './resources/views/temp_caches/';
        protected $vars = [];
        protected $dataView = []; 
        ...
        public function render($template_file, $data = false) 
        {
            ...
            if (file_exists($this->template_dir.$template_file.self::EXT)) {
                extract($this->vars);
                ob_start();
                require_once $this->template_dir.$template_file.self::EXT;
                $rendered = ob_get_contents(); 
                ob_end_flush();  
            } else {
                
                throw new Exception('no template file ' . $template_file.self::EXT . ' present in directory ' . $this->template_dir);
            }
            $this->dataView = [];
        }
        ...
        public function extendsss($template = null)
        {
            if ($template) {
                include_once($this->template_dir.$template.self::EXT);
            }
        }
        ...
        public function extend($template = null)
        {
            if ($template) {
                ...
                if (file_exists($this->template_dir.$template.self::HAX)) {
                    // compile the hax template and include it if exists
                    $file = $this->renderHaxTemplate($template);
                } elseif (file_exists($this->template_dir.$template.self::EXT)) {
                    $file = $this->template_dir.$template.self::EXT;
                    include_once($file);
                } else {
        ...
        function newSection($name, $filters, $trace) 
        {
            $base =& $GLOBALS['_smx_base'];
            $stack =& $GLOBALS['_smx_stack'];
